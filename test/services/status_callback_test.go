...
func TestStatusCallbackInsertsArchivedRecordDeletesQueuedRecord(t *testing.T) {
	...
	err := services.HandleStatusCallback(qj.ID, "echo", models.ArchivedJobStatusSucceeded, 7, true)
...
func testStatusCallbackFailedInsertsArchivedRecord(t *testing.T) {
	...
	err := services.HandleStatusCallback(qj.ID, job.Name, models.ArchivedJobStatusFailed, 1, true)
	test.AssertNotError(t, err, "")
	_, err = queued_jobs.Get(qj.ID)
	test.AssertEquals(t, err, queued_jobs.ErrNotFound)
	aj, err := archived_jobs.Get(qj.ID)
...
func TestStatusCallbackFailedAtMostOnceInsertsArchivedRecord(t *testing.T) {
	...
	err := services.HandleStatusCallback(qj.ID, "at-most-once", models.ArchivedJobStatusFailed, 7, true)
	test.AssertNotError(t, err, "")
	_, err = queued_jobs.Get(qj.ID)
	test.AssertEquals(t, err, queued_jobs.ErrNotFound)
	aj, err := archived_jobs.Get(qj.ID)
...
func testStatusCallbackFailedAtLeastOnceUpdatesQueuedRecord(t *testing.T) {
	t.Parallel()
	job, qj := factory.CreateUniqueQueuedJob(t, factory.EmptyData)
	err := services.HandleStatusCallback(qj.ID, job.Name, models.ArchivedJobStatusFailed, 7, true)
	test.AssertNotError(t, err, "")
...
func testStatusCallbackFailedNotRetryableArchivesRecord(t *testing.T) {
	...
	err := services.HandleStatusCallback(qj.ID, qj.Name, models.ArchivedJobStatusFailed, qj.Attempts, false)
...
func TestStatusCallbackFailedAtMostOnceArchivedRecordExists(t *testing.T) {
	defer test.TearDown(t)
	aj := factory.CreateArchivedJob(t, factory.EmptyData, models.ArchivedJobStatusFailed)
	err := services.HandleStatusCallback(aj.ID, aj.Name, models.ArchivedJobStatusFailed, 1, true)
	test.AssertEquals(t, err, queued_jobs.ErrNotFound)
}
